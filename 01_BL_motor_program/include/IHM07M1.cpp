...
void IHM07M1::begin()
{
    ...
    /* softwarePWM parameter */
    gpioSetPWMrange(IN1, 20000);
    gpioSetPWMrange(IN2, 20000);
    gpioSetPWMrange(IN3, 20000);
...
void IHM07M1::outputUVWVoltage(vector<double> Vuvw)
{
   ...
   */
   ...
   outputSoftwarePWM(IN1, Vuvw[0]);
   outputSoftwarePWM(IN2, Vuvw[1]);
   outputSoftwarePWM(IN3, Vuvw[2]);
...
void IHM07M1::outputSoftwarePWM(int IN_No, double voltage)
{
    double voltage_buf;
    ...
    int MOD_F_value = static_cast<int>(20000.0 * voltage_Inv/supply_voltage);

    gpioPWM(IN_No, MOD_F_value);
}