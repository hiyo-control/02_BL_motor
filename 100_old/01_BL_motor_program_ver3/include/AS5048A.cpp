...
uint16_t AS5048A::read(uint16_t registerAddress)
{
	...
	unsigned char dataAngle[2]    = {dataRW_lower, dataRW_upper};
	...
	if (wiringPiSPIDataRW(this->channel, dataAngle, sizeof(dataAngle)) == -1)
	{
	...
	uint16_t response_upper = (static_cast<uint16_t>(dataAngle[0] << 8));
	uint16_t response_lower = (static_cast<uint16_t>(dataAngle[1]));
...
{
	int16_t rotation = getRotation();
	double radian = 2*M_PI * (rotation + AS5048A_MAX_VALUE) / (AS5048A_MAX_VALUE * 2.0);
	return radian;
}
